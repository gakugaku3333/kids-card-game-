/*
 * core/firelog.js — 誤答ログ・間隔反復・学習履歴（ESM版）
 *
 * js/firebase-log.js と同一のFirestoreスキーマを読み書きする。
...
const NICK_KEY = 'kids_game_nickname';
let db = null;
...
export function setUser(nickname) {
  ...
  if (ref) {
    ...
    }, { merge: true }).catch((e) => console.warn('[FireLog] setUser:', e));
  }
}
...
export function logWrong(game, question, answer) {
  ...
  }).catch((e) => console.warn('[FireLog] logWrong:', e));
}
...
export function markReviewed(game, question) {
  ...
  }).catch((e) => console.warn('[FireLog] markReviewed:', e));
}
...
export function getReviewDue(callback) {
  ...
  ref.collection('wrongItems')
    ...
    .catch((e) => { console.warn('[FireLog] getReviewDue:', e); callback([]); });
}
...
export function logSession(game, correct, total, tokens) {
  ...
  }).catch((e) => console.warn('[FireLog] logSession:', e));
}
...
export function getSessions(limit, callback) {
  ...
  ref.collection('sessions')
    ...
    .catch((e) => { console.warn('[FireLog] getSessions:', e); callback([]); });
}
