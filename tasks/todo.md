...
[リニューアル指示書.md](リニューアル指示書.md) の残タスクのうち、`js/store.js`系レガシーからの移行が容易な3ゲーム（memory/catch/draw）を対応。

### games/memory/（`core/shell.js`導入）
- ヘッダーの「もどる」ボタンはシェル製の🏠ハブ導線に統一。ただし**プレイ中の「レベルせんたくへ」ボタン**（ゲーム内で難易度選択に戻る機能。ハブに戻るのとは別物）は独自に維持し、ラベルを明確化して混同を防止
- `window.Store.addTokens` → `shell.store.addTokens`、`shell.fireLog.logSession('memory', ...)` を追加（今まで学習履歴に記録されていなかった）
- 独自の効果音（めくる/ペア成立/ハズレ/ファンファーレのWeb Audio合成）と独自の結果モーダル（タイム・ミス数表示）は維持（shellの汎用モーダルより情報量が多いため）
- 検証: かんたん(6枚)を実際にマッチさせクリア→⭐5加算・ヘッダー即時反映・タイム/ミス数の結果表示・レベル選択への「もどる」動作を確認

### games/catch/（`core/shell.js`へ全面移行）
- 独自ヘッダー・独自結果モーダルを撤去し、`shell.showResult({correct, total:0, tokens})`に統一
- 冪等ガード（`cancelAnimationFrame`）は既存の自己管理パターンを維持（shellのタイマー追跡は使わず、元々あった仕組みをそのまま活かす方が安全と判断）
- **検証の制約**: このゲームは`requestAnimationFrame`でループしており、previewブラウザのタブが`document.hidden`（非表示）だとrAFが発火せず自動検証できない。ヘッダー・スタート画面・結果モーダルの表示構造は確認したが、実際のキャッチ判定はiPad実機での確認が必要

### games/draw/（ヘッダーのみ`core/shell.js`化）
- 独自トップバー（タイトル+ホームボタン）を撤去し、shellヘッダー（トークンバッジ付き、drawには元々トークン表示が無かった）に統一
- `window.Store.addTokens` → `shell.store.addTokens`
- 結果モーダルの概念が無いゲームのため`shell.showResult`は未使用（採点ではなく「できた！」トースト通知の仕組みをそのまま維持）
- 検証: 実際に描画→「できた！」→⭐3加算→ヘッダー反映→ハブに戻って同じトークン数が共有されていることを確認

### 意図的に見送った範囲
- `index.html`（ハブ）・`games/rakugaku/*`・`games/mole/`は今回未着手。ハブは`js/shop.js`/`js/avatar.js`のcore/移行が別途必要（該当するcore/ui/モジュール未実装）、rakugakuはクイズエンジン統合とセットで行うべき大改修、moleは前回セッションで別の理由（2人対戦追加）で改修済みのため今回は触らずデグレ回避を優先

---

# タスク完了: 新ゲーム3本追加（とけいよみ・2人対戦もぐらたたき・おかいものやさん）

## 完了日: 2026-07-04

[新ゲーム設計図.md](新ゲーム設計図.md) の3本を実装。`games/_template/` で検証済みの `core/shell.js` を活用。

### 1. ⏰ とけいよみクイズ（新規: `games/clock/`）
- `clock-face.js` — アナログ時計のSVG描画。短針は分に応じて中間位置まで傾く（3:30なら3と4のちょうど中間、preview で角度を数値検証済み）
- 3レベル（なんじ？/なんじはん？/なんじなんぷん？）、1プレイ10問、`core/shell.js` 経由でトークン・リザルト・誤答ログ(`FireLog.logWrong`)
- ふん/ぷんの使い分け表を実装し、5分刻み11パターンすべて出力を確認（例: 15→じゅうごふん、20→にじゅっぷん）
- 誤答選択肢は「時間を±1時間ずらす」「分を別の値にする」を組み合わせて生成

### 2. 🔨 2人たいせんもぐらたたき（既存 `games/mole/` を改修）
- ロジックを `MoleBoard` クラスに抽出（1人=インスタンス1個、2人=2個）
- スタート画面に「ひとりで」「ふたりで」を追加。ひとりでモードは既存挙動そのまま（回帰確認済み）
- ふたりでモード: 上盤面を180度回転、60秒、両盤面に同一マス・同一種別を同時スポーンさせて公平性を担保、`pointerdown`で同時タップに対応
- 終了時: 勝者⭐3・敗者⭐1・引き分け⭐2×2 を合算し `Store.addTokens` を1回だけ呼ぶ（共有ウォレットのため二重加算しない設計）
- **iPad実機での2点同時タップ検証は未実施**（preview環境では検証不可）

### 3. 🛒 おかいものやさん（新規: `games/shopping/`）
- `items.json`（15品）+ `game.js`（ステージA〜Cの状態管理）+ 単独ページ
- ステージA: 棚タップ→カゴ→たしざん問題→キーボード入力
- ステージB: SVG自作硬貨（1/5/10/50/100円、5円・50円は穴あき表現）をタップでトレイへ移動→金額一致で自動正解（複数の支払いパターンを自然に許容）
- ステージC（レベル2以降）: 「〇〇円ではらったらおつりは？」の暗算
- 3回のおつかい完了で ⭐2×3 + コンプリートボーナス⭐4 = 最大⭐10（preview で実際に⭐10獲得を確認）
- 誤答ログ: `shopping-add` / `shopping-change` で記録

### games.json 更新
- `mole` のタイトルを「もぐらたたき 🔨（ふたりでも！）」に更新
- `clock-1`・`shopping` を新規追加（`"new": true`）、サムネイルSVGを新規作成

### 検証（詳細は [smoke-test.md](smoke-test.md)）
previewで3本とも一連のプレイフローをコンソールエラーなしで確認。横向き（926×428）でとけいよみのレイアウト崩れなしをスクリーンショットで確認。**もぐらたたきの2人同時タップのみiPad実機確認が必要（次回対応）**。
