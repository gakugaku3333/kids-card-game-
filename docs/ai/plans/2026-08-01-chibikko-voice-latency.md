...
- **状態**: 実装済み（Chrome側AC1〜9は確認済み。HC1〜3は実機で確認され、そこで見つかった
  「あいさつと最初の出題音声が重なる」問題も修正済み。この修正後の再確認は未実施のため
  `implemented`は保留）
...
- 2026-08-02: 実機（ユーザーによるiPad/iPhone Safari確認）で新たな問題を発見・修正
  （Claude・別セッション）。あいさつ音声の直後、`pick/engine.js`が`_next()`内で
  `setTimeout(() => speak(askVoiceId), 300)`により最初の出題音声を鳴らすため、
  あいさつが鳴り終わる前（300ms後）に出題音声が重なって聞き取れなくなっていた。
  `voice.js`の`speak()`を、再生完了（`ended`イベント、フォールバック時は
  `SpeechSynthesisUtterance`の`end`/`error`）で解決するPromiseを返すよう変更し、
  `shell.js`の`autoStart()`のタップハンドラで`Voice.speak(greeting)`の完了
  （4秒の安全タイムアウト付き）を待ってから`startGame(onStart)`を呼ぶようにした。
  **`#33`（重なりを許容し、キュー・完了待ちを再導入しない）を覆すものではなく、
  あいさつ→最初の出題という一箇所だけの意図的な例外。** それ以外（正誤演出・
  ほめ言葉・リザルト等）は従来どおり重ねて鳴らす。Chromeで`HTMLMediaElement.play`を
  フックして計測し、あいさつ再生開始からおよそ3.1秒後（あいさつの再生完了後）に
  出題音声が始まることを確認した。HC1〜HC3はユーザー実機で確認済み
  （このタイミング問題の指摘自体がその確認から得られたもの）。
